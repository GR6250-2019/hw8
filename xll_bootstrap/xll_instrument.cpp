...
AddIn xai_instrument_fra(
	...
	.Arg(XLL_DOUBLE, L"effective", L"is the time in years at which the forward rate argeement starts.")
...
AddIn xai_instrument_swap(
	Function(XLL_HANDLE, L"?xll_instrument_cd", CATEGORY L".INTEREST_RATE_SWAP")
	.Arg(XLL_DOUBLE, L"maturity", L"is the time in years at which the interest rate swap matures.")
	.Arg(XLL_DOUBLE, L"frequency", L"is the number of coupons per year")
	.Arg(XLL_DOUBLE, L"coupon", L"is the annual nominal coupons in percentage of the par value")
	.Uncalced()
	.Category(CATEGORY)
...
HANDLEX WINAPI xll_instrument_swap(double maturity, int frequency, double coupon)
{
#pragma XLLEXPORT
	handlex result;
