...
AddIn xai_instrument_fra(
	...
	.Arg(XLL_DOUBLE, L"effective", L"is the time in years at which the forward rate agreement takes effect.")
	.Arg(XLL_DOUBLE, L"tenor", L"is the remaining time to maturity in years.")
	.Arg(XLL_DOUBLE, L"forward", L"forward")
	.Uncalced()
	.Category(CATEGORY)
	.FunctionHelp(L"Return a handle to a forward rate agreement instrument.")
	.Documentation(
		L"Forward rate agreements have two cash flows : -1 at the effective date" delta_
...
AddIn xai_instrument_swap(
	...
	.Arg(XLL_DOUBLE, L"maturity", L"is the time in years at which the interest rate swap ends.")
	.Arg(XLL_DOUBLE, L"frequency", L"is the number of coupons per year.")
	.Arg(XLL_DOUBLE, L"coupon", L"is the interest payment you paid for an interest rate swap. ")
	.Uncalced()
	.Category(CATEGORY)
	.FunctionHelp(L"Return a handle to an interest rate swap instrument.")
	.Documentation(
		L"A swap is determined by (maturity, frequency, coupon). It has cashflows" delta_
		L"-1 at 0, coupon/frequency, at times i/frequency for i = 1, 2, ..., n - 1," delta_
