...
AddIn xai_instrument_fra(
	...
	.Arg(XLL_DOUBLE, L"effective", L"is the date at which the rate becomes effective.")
	.Arg(XLL_DOUBLE, L"tenor", L"is the time in years at which the FRA matures.")
	.Arg(XLL_DOUBLE, L"forward", L"is the forward rate.")
	.Uncalced()
	.Category(CATEGORY)
	.FunctionHelp(L"Return a handle to a forward rate argreement instrument.")
	.Documentation(
		L"A forward rate agreement has two cash flows. The first is at time when it's effective and is always -1. "
		L"This corresponds to having initial price 1. "
		L"The second occurs at " C_(L"tenor") L" and is equal to  1 + r" delta_
		L" where r is the forward " C_(L"rate") L" and " delta_
		L" is the " C_(L"tenor") L" in years. "
	)
);
...
AddIn xai_instrument_swap(
	...
	.Arg(XLL_DOUBLE, L"coupon", L"is the value of the coupon.")
	.Uncalced()
	.Category(CATEGORY)
	.FunctionHelp(L"Return a handle to a interest rate swap instrument.")
	.Documentation(
		L"A interest rate swap has (1 + frequency*maturity) cash flows."
		L"It has cash flows at -1 at time 0, coupon/frequency at times "
		L"i/frequency for i = 1, 2, ..., n-1" 
		L"and 1 + coupon/frequence at maturity = n/frequency." 
		L"Time is measured in years. "
	)
);
