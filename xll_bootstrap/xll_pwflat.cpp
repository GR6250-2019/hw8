...
AddIn xai_pwflat_forward_spot(
	...
	.Arg(XLL_HANDLE, L"forward", L"is a handle to a piecewise flat forward.")
	.Arg(XLL_FP, L"times", L"is an array of times at which to find the forward values.")
	.Category(CATEGORY)
	.FunctionHelp(L"Return an array of spot values corresponding to times. ")
	.Documentation(
...
#pragma XLLEXPORT
	...
	try {
		...
		result.resize(rows(*pt), columns(*pt));
		for (int i = 0; i < size(*pt); ++i) {
			result[i] = fwd_->value(pt->array[0]);//pointer to spot value
		}
	}
	catch (const std::exception& ex) {
		XLL_ERROR(ex.what());
...
AddIn xai_pwflat_forward_discount(
	...
	.Arg(XLL_FP, L"times", L"is an array of times at which to find the forward values.")
	.Category(CATEGORY)
	.FunctionHelp(L"Return an array of the discount values corresponding to times.")
	.Documentation(
		L"Return an array of discount values corresponding to times."
	)
);
...
#pragma XLLEXPORT
	...
	try {
		...
		for (int i = 0; i < size(*pt); ++i) {
			...
			result[i] = fwd_->value(pt->array[i] / pt->array[0]);//pointer to spot value
