...
#include "xll_bootstrap.h"#ifdef CATEGORY
#undef CATEGORY
#endif
#define CATEGORY L"PWFLAT"using namespace xll;AddIn xai_pwflat(
	Document(CATEGORY)
	.Category(CATEGORY)
	.Documentation(
		L"Functions for piecewise flat forward curves. "
	)
); using forward = fms::pwflat::forward<fms::sequence::list<double>, fms::sequence::list<double>>; AddIn xai_pwflat_forward(
...
#pragma XLLEXPORT
	handlex result; try {
		ensure(size(*pt) == size(*pf));  handle<forward> forward_(new forward(list(*pt), list(*pf)));  result = forward_.get();
	}
...
#pragma XLLEXPORT
	static xll::FP12 result; try {
		handle<forward> fwd_(fwd);  result.resize(rows(*pt), columns(*pt));
...
AddIn xai_pwflat_forward_spot(
	...
	.Category(CATEGORY)
	.FunctionHelp(L"Return an array of forward spot corresponding to times. ")
	.Documentation(
		L"Return an array of forward spot corresponding to times. "
	)
);
...
AddIn xai_pwflat_forward_discount(
	...
	.Category(CATEGORY)
	.FunctionHelp(L"Return an array of forward discount corresponding to times. ")
	.Documentation(
		L"Return an array of forward discount. "
	)
);
