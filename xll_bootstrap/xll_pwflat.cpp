...
#pragma XLLEXPORT
	...
	try {
		...
		for (int i = 0; i < size(*pt); ++i) {
			result[i] = fwd_->value(pt->array[i]);///PT=0//FORWARD:SPOT
		}
	}
	catch (const std::exception & ex) {
		XLL_ERROR(ex.what());
...
#pragma XLLEXPORT
	...
	try {
		...
		result.resize(rows(*pt), columns(*pt));
		for (int i = 0; i < size(*pt); ++i) {
			result[i] = fwd_->value(pt->array[0]);///PT=0//FORWARD:SPOT
		}
	}
	catch (const std::exception & ex) {
		XLL_ERROR(ex.what());
...
#pragma XLLEXPORT
	...
	try {
		...
		result.resize(rows(*pt), columns(*pt));
		for (int i = 0; i < size(*pt); ++i) {
			result[i] = fwd_->value(pt->array[i]/pt->array[0]);///PT=0//FORWARD:SPOT
		}
	}
	catch (const std::exception & ex) {
		XLL_ERROR(ex.what());
