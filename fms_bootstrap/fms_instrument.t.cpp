...
{
	forward_rate_agreement<U, C> fra(0, 2, 3);
...
{
	interest_rate_swap<U, C> swap(2,2, 3);
	...
	++swap;
	assert(std::pair(1/2, 3/2) == *swap);
	++swap;
	assert(std::pair(1, 3 / 2) == *swap);
	++swap;
	assert(std::pair(3 / 2, 3 / 2) == *swap);
	++swap;
	assert(std::pair(2,1 + 3 / 2) == *swap);
	++swap;
	assert(!swap);
