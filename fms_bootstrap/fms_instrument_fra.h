...
// and 1 + forward*tenor at the expiration date = effective + tenor.
...
namespace fms::instrument {
	...
	template<class B = double, class U = double, class C = double>
	struct forward_rate_agreement : public sequence<fms::sequence::list<U>, fms::sequence::list<C>> {
		forward_rate_agreement(B effective, U tenor, C forward)
