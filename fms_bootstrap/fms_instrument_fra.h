...
namespace fms::instrument {
	...
	struct fra_contract : public sequence<fms::sequence::list<U>, fms::sequence::list<C>> {
		fra_contract(U effective, U tenor, C forward)
			: sequence<fms::sequence::list<U>, fms::sequence::list<C>>(
				fms::sequence::list<U>({ 0, effective, tenor }), fms::sequence::list<C>({ 0,-1, 1 + forward * tenor })
				)
		{ }
	};
}